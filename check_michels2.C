...
void check_michels2(char *filename=NULL) {
  ...
  TH1D *charge = new TH1D("charge","charge per PMT", 400, -0.5, 799.5);
  TH1D *maxcharge = new TH1D("maxcharge","Maximum charge per event", 50, -0.5, 799.5);
  TH1D *ccharge = new TH1D("ccharge","charge per PMT", 400, -0.5, 799.5);
  TH1D *cmaxcharge = new TH1D("cmaxcharge","Maximum charge per event", 100, -0.5, 799.5);
  ...
  TH2D *highPE_XZ = new TH2D("highPE_XZ","High PE Photons Origin",600,-3,3,1000,-5,5);
  highPE_michelXZ->SetXTitle("Location x(m)");
  highPE_michelXZ->SetYTitle("Location z(m)");
  ...
  for(int ii = 0; ii < nevent; ii++){
    ...
    for(int i = 0; i < wcsimrootevent->GetNtrack(); i++){
      ...
      if(1)      std::cout << "particle type: " <<  track->GetIpnu()
		<< " " << track->GetFlag()
		<< " " << track->GetM()
		<< " " << track->GetP()
		<< " " << track->GetE()		
    ...
    */
    ...
    for (int i = 0; i<max; i++){
      ...
      for(int k = 0; k < chit->GetTotalPe(1); k++) {
	WCSimRootCherenkovHitTime *cthit = (WCSimRootCherekovHitTime*) wcsimrootevent->GetCherenkovHitTimes()->At(chit->GetTotalPe(0) + k);
    ...
    if(clargest > 100) {
      ...
      for(i=0;i<3;i++) 
	std::cout << track->GetStart(i) << " ";

      highPE_XZ->Fill(track->GetStart(0),track->GetStart(2));
  ...
  TCanvas *cLoc = new TCanvas("cloc","cLoc",3*350,5*225);
  highPE_XZ->Draw("colz");
