...
void check_michels(char *filename=NULL) {
  ...
  TH2D *highPE_XZ = new TH2D("highPE_XZ","High PE Particle Origins",20,-3,3,20,-5,5);
  highPE_XZ->SetXTitle("Location x(m)");
  highPE_XZ->SetYTitle("Location z(m)");
  ...
  for(int ii = 0; ii < nevent; ii++){
    ...
    */
    ...
    if(clargest > 200) {
      ...
      highPE_XZ->Fill(track->GetStart(0)/100.,track->GetStart(2)/100.);
